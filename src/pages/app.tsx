...
import {
  easyTitles as easyGateBlocksTitles,
  hardTitles as hardGateBlocksTitles
} from '../blockly/blocks/gateblocks'
...
export default function AppContainer ({
  ...
  const [gateBlocks, setGateBlocks] = useState<React.ReactNode[] | null>([
    <Block type={'ccx_gate'} key={`gate ${1}`} />
  ])
  ...
  useEffect(() => {
    ...
    const newBlocks =
      difficulty === Difficulty.EASY
        ? easyGateBlocksTitles.map((title, index) => (
  ...
  if (mode === 'inspect') {
    ...
    circuit = new QuantumCircuit(3)
    circuit.importQASM('OPENQASM 2.0;', (err: string[]) => {
      if (err && err.length > 0) return console.log('error', '', err)
    })
    svg = circuit.exportSVG(true)
  }
  ...
  Blockly.Themes.Classic.categoryStyles.gate_category =
    categoryStyles.gate_category
  Blockly.Themes.Classic.categoryStyles.variable_category =
    categoryStyles.variable_category
  Blockly.Themes.Classic.categoryStyles.other_category =
    categoryStyles.other_category
    ...
    <div className={styles.container}>
      <div className={styles.appcontainer}>
        <div
          ...
          style={{ display: mode === 'd&d' ? 'block' : 'none' }}
        >
          <BlocklyComponent
            setQASM={(qasm: string) => {
              setQasmBox(qasm)
          ...
          >
            <Category
              name='Built-in Gate Blocks'
              categoryStyle={categoryNameToStyle['Gate']}
            >
              {gateBlocks}
            </Category>
            <Category
              name='Variable Blocks'
              categoryStyle={categoryNameToStyle['Var']}
            >
            ...
            <Category
              name='Other Blocks'
              categoryStyle={categoryNameToStyle['Other']}
            >
        ...
        <CodeArea
          visible={mode === 'inspect'}
          code={qasmBox}
          showCircuit={showPopup}
        ></CodeArea>
      </div>
